...
import { 
  ...
  ShieldCheck, Plus, CheckCircle, Truck, Phone, Mail, User 
} from 'lucide-react';
...
export default function Checkout() {
  const { cartItems, cartTotal, cartSubtotal, cartDelivery, clearCart } = useData();
  const [savedAddresses, setSavedAddresses] = useState([]);
  const [selectedAddrId, setSelectedAddrId] = useState(null);
  const [form, setForm] = useState({
    firstName: '', lastName: '', email: '', phone: '',
    address: '', city: '', postalCode: '', payment: 'razorpay'
  });
    ...
    if (form.payment === 'razorpay') {
      try {
        ...
        const options = {
          ...
          handler: async (response) => finalizeOrder(response.razorpay_payment_id),
          prefill: {
  ...
  return (
    <div className="checkout-page container section">
      ...
      <div className="checkout-grid">
        <div className="checkout-main animate-fadeUp" style={{ animationDelay: '0.1s' }}>
          ...
          <section className="checkout-section glass">
            ...
            <div className="address-selection-grid">
              {savedAddresses.map((addr) => (
                ...
                >
                  <div className="card-check"><CheckCircle size={18} /></div>
          ...
          <section className="checkout-section glass">
            ...
            <div className="payment-options">
              <label className={`payment-option ${form.payment === 'razorpay' ? 'active' : ''}`}>
          ...
          {orderError && <div className="checkout-error-msg animate-shake">{orderError}</div>}
        </div>

        {/* Sidebar Summary */}
        <aside className="checkout-sidebar animate-fadeUp" style={{ animationDelay: '0.2s' }}>
          <div className="summary-card-checkout glass">
            ...
            <div className="summary-items-mini">
              {cartItems.map(item => (
                <div key={item.id} className="mini-item">
                  ...
                  <div className="mini-price">{formatPrice(item.price * item.quantity)}</div>
                </div>
              ))}
            </div>
            
            <div className="summary-details">
              <div className="detail-row"><span>Subtotal</span><span>{formatPrice(cartSubtotal)}</span></div>
              <div className="detail-row"><span>Delivery</span><span>{cartDelivery === 0 ? 'FREE' : formatPrice(cartDelivery)}</span></div>
              <div className="divider"></div>
              <div className="detail-row total"><span>Total Amount</span><span>{formatPrice(cartTotal)}</span></div>
            </div>
            ...
            <div className="checkout-trust">
              <div className="trust-badge"><ShieldCheck size={16} /> 256-bit SSL Secure</div>
              <div className="trust-badge"><Truck size={16} /> Tracked Delivery</div>
            </div>
          </div>
        </aside>
      </div>
