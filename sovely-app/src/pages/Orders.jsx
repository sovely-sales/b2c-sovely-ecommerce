...
import { ShoppingBag, ChevronRight, Package, Calendar, CreditCard, ExternalLink } from 'lucide-react';
import './Orders.css';
...
export default function Orders() {
  ...
  if (loading) return <div className="orders-loading"><div className="loader"></div></div>;
  ...
  return (
    <div className="orders-page container section">
      ...
      <div className="orders-list animate-fadeUp" style={{ animationDelay: '0.1s' }}>
        {orders.length === 0 ? (
            ...
            <div key={order._id} className="order-card glass">
              ...
              <div className="order-card-footer">
                <div className="order-payment-info">
                  <CreditCard size={16} />
                  <span>{order.paymentMethod === 'razorpay' ? 'Paid via Razorpay' : 'Cash on Delivery'}</span>
                </div>
                <div className="order-total-block">
                  <span className="label">Total Amount</span>
                  <span className="total-value">{formatPrice(order.total)}</span>
                </div>
                <button className="btn btn-outline track-btn" onClick={() => navigate('/track')}>
